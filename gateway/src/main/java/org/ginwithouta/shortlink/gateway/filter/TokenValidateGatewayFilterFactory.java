package org.ginwithouta.shortlink.gateway.filter;

import com.alibaba.fastjson2.JSON;
import com.alibaba.fastjson2.JSONObject;
import org.ginwithouta.shortlink.gateway.config.Config;
import org.ginwithouta.shortlink.gateway.dto.GatewayErrorResult;
...
public class TokenValidateGatewayFilterFactory extends AbstractGatewayFilterFactory<Config> {
    private final String REDIS_USER_ALREADY_LOGIN_IN_KEY = "short-link:login:";
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            if (!isPathInWhiteList(requestPath, requestMethod, config.getWhitePathList())) {
                ...
                Object userInfo;
                if (StringUtils.hasText(username) && StringUtils.hasText(token) && (userInfo = stringRedisTemplate.opsForHash().get(REDIS_USER_ALREADY_LOGIN_IN_KEY + username, token)) != null) {
